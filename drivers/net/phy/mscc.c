...
static int mscc_phy_soft_reset(struct phy_device *phydev)
{
  ...
  debug("MSCC try reading register\n");
	reg_val = phy_read(phydev, MDIO_DEVAD_NONE, MII_BMCR);

	while ((reg_val & BMCR_RESET) && (timeout > 0)) {
		debug("MSCC try reading register again\n");
		reg_val = phy_read(phydev, MDIO_DEVAD_NONE, MII_BMCR);
		timeout--;
		udelay(1000);   /* 1 ms */
	}
...
static int vsc8531_vsc8541_mac_config(struct phy_device *phydev)
{
	...
	u16	mac_if = 0;
	u16	rx_clk_out = 0;
  debug("vsc8531_vsc8541_mac_config\n");
...
static int vsc8531_config(struct phy_device *phydev)
{
	...
	enum vsc_phy_rgmii_skew  tx_clk_skew = VSC_PHY_RGMII_DELAY_800_PS;//this
	enum vsc_phy_clk_slew    edge_rate = VSC_PHY_CLK_SLEW_RATE_7;
...
int vsc8531_get_error_ctr(struct phy_device *phydev) {
  return phy_read(phydev, MDIO_DEVAD_NONE, 0x13);
}

int vsc8531_get_error_100(struct phy_device *phydev) {
  return phy_read(phydev, MDIO_DEVAD_NONE, 0x10);
}

int vsc8531_get_error_1000(struct phy_device *phydev) {
  return phy_read(phydev, MDIO_DEVAD_NONE, 0x11);
}

int vsc8531_get_mmd(struct phy_device *phydev) {
  return phy_read(phydev, MDIO_DEVAD_NONE, 0x0e);
}

int vcs8531_get_cntl_reg(struct phy_device *phydev) {
  int reg_val = 0;
  phy_write(phydev, MDIO_DEVAD_NONE, MSCC_EXT_PAGE_ACCESS,
            MSCC_PHY_PAGE_EXT2);
