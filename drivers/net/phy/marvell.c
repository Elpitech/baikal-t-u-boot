...
static int m88e1518_config(struct phy_device *phydev)
{
	...
	if (phydev->interface == PHY_INTERFACE_MODE_SGMII) {
		printf("%s: PHY_INTERFACE_MODE_SGMII\n", __func__);
		/* Select page 18 */
		...
		reg = phy_read(phydev, MDIO_DEVAD_NONE, MIIM_88E1111_PHY_EXT_SR);

		reg &= ~(MIIM_88E1111_HWCFG_MODE_MASK);
		reg |= MIIM_88E1111_HWCFG_MODE_SGMII_NO_CLK;
		reg |= MIIM_88E1111_HWCFG_FIBER_COPPER_AUTO;

		phy_write(phydev, MDIO_DEVAD_NONE, MIIM_88E1111_PHY_EXT_SR, reg);
	}
	...
	if (phy_interface_is_rgmii(phydev)) {
		...
		if (phydev->interface == PHY_INTERFACE_MODE_RGMII ||
			phydev->interface == PHY_INTERFACE_MODE_RGMII_ID) {
			printf("%s: MIIM_88E151x_RGMII_RXTX_DELAY\n", __func__);
			reg |= MIIM_88E151x_RGMII_RXTX_DELAY;
		}
		else if (phydev->interface == PHY_INTERFACE_MODE_RGMII_RXID) {
			printf("%s: MIIM_88E151x_RGMII_RX_DELAY\n", __func__);
			reg |= MIIM_88E151x_RGMII_RX_DELAY;
		}
		else if (phydev->interface == PHY_INTERFACE_MODE_RGMII_TXID) {
			printf("%s: MIIM_88E151x_RGMII_TX_DELAY\n", __func__);
