 ...
 *
 *  Parameters:
 *      value    - Bit value to set to the SCL or SDA (0 = low, 1 =
 high)
 *
 *  Notes:
 *      When setting SCL to high, just set the GPIO as input where the
 pull up
 *      resistor will pull the signal up. Do not use software to pull
 up the
 *      signal because the i2c will fail when other device try to
 drive the
                             ...
                             unsigned char data_gpio)
{
  ...
  /* Note that SM750LE don't have GPIO MUX and power is always
     on */
  /* Clear the i2c lines. */
  for (i = 0; i < 9; i++)
    sw_i2c_stop();
  ...
  )
{
  ...
  /* Get the bus again and get the data from the device read
     address */
  sw_i2c_start();
  ...
  )
{
  ...
  /* Send the device address and read the data. All should
     return success
