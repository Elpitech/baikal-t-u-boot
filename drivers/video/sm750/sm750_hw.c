...
int hw_sm750_map(struct sm750_dev *sm750_dev, pci_dev_t devbusfn)
{
  int ret;
  ...
  pci_read_config_dword(devbusfn, PCI_BASE_ADDRESS_1,
                        (unsigned int *)&sm750_dev->vidreg_start);
  sm750_dev->vidreg_start &= PCI_BASE_ADDRESS_MEM_MASK;
  sm750_dev->vidreg_size = SZ_2M;

  pr_info("mmio phyAddr = %lx\n", sm750_dev->vidreg_start);

  /* now map mmio and vidmem*/
  sm750_dev->pvReg = (void *)(unsigned
                              long)pci_mem_to_phys(devbusfn, sm750_dev->vidreg_start);
  if (!sm750_dev->pvReg) {
    pr_err("mmio failed\n");
    ret = -EFAULT;
    goto exit;
  ...
  ddk750_set_mmio(sm750_dev->pvReg, sm750_dev->devid,
                  sm750_dev->revid);
  pci_read_config_dword(devbusfn, PCI_BASE_ADDRESS_0, (unsigned int *)&sm750_dev->vidmem_start);
  sm750_dev->vidmem_start &= PCI_BASE_ADDRESS_MEM_MASK;
  ...
  sm750_dev->pvMem = (void *)(unsigned long)pci_mem_to_phys(devbusfn, sm750_dev->vidmem_start);
  if (!sm750_dev->pvMem) {
...
int hw_sm750_inithw(struct sm750_dev *sm750_dev)
{
  ...
  if (getChipType() != SM750LE) {
    ...
    /* for 750LE ,no DVI chip initilization makes Monitor
       no signal */
    /* Set up GPIO for software I2C to program DVI chip in
       the
...
#ifdef USE_DVICHIP
  if(dviInit(1, 1, 0, 1, 1, 1, 7, 1, 1, 4))
    pr_info("Init Display failed\n");
#endif
  return 0;
}
                            ...
                            fb_var_screeninfo *var)
{
  ...
  struct lynxfb_par *par = container_of(crtc, struct lynxfb_par,
                                        crtc);
  sm750_dev = par->dev;
                          ...
                          fb_fix_screeninfo *fix)
{
  ...
  modparm.vertical_sync_polarity = (var->sync &
                                    FB_SYNC_HOR_HIGH_ACT) ? POS:NEG;
  modparm.horizontal_sync_polarity = (var->sync &
                                      FB_SYNC_VERT_HIGH_ACT) ? POS:NEG;
  modparm.clock_phase_polarity = (var->sync &
                                  FB_SYNC_COMP_HIGH_ACT) ? POS:NEG;
  ...
  if (crtc->channel != sm750_secondary) {
    ...
    POKE32(PANEL_FB_ADDRESS,
           FIELD_SET(0, PANEL_FB_ADDRESS, STATUS,
                     CURRENT)|
           FIELD_SET(0, PANEL_FB_ADDRESS, EXT, LOCAL)|
           FIELD_VALUE(0, PANEL_FB_ADDRESS, ADDRESS,
                       crtc->oScreen));
    reg = var->xres * (var->bits_per_pixel >> 3);
    /* crtc->channel is not equal to par->index on
       numeric,be aware of that */
    ...
    POKE32(PANEL_WINDOW_WIDTH,
           FIELD_VALUE(0, PANEL_WINDOW_WIDTH, WIDTH,
                       var->xres - 1)|
           FIELD_VALUE(0, PANEL_WINDOW_WIDTH, X,
                       var->xoffset));
    POKE32(PANEL_WINDOW_HEIGHT,
           FIELD_VALUE(0, PANEL_WINDOW_HEIGHT, HEIGHT,
                       var->yres_virtual - 1)|
           FIELD_VALUE(0, PANEL_WINDOW_HEIGHT, Y,
                       var->yoffset));
    ...
    reg = FIELD_VALUE(reg, CRT_DISPLAY_CTRL, FORMAT,
                      var->bits_per_pixel >> 4);
...
{
  static unsigned int add[] = {PANEL_PALETTE_RAM,
                               CRT_PALETTE_RAM};
...
int hw_sm750le_setBLANK(struct lynxfb_output *output, int blank)
{
  ...
  if (output->paths & sm750_crt) {
    POKE32(CRT_DISPLAY_CTRL,
           FIELD_VALUE(PEEK32(CRT_DISPLAY_CTRL), CRT_DISPLAY_CTRL, DPMS, dpms));
    POKE32(CRT_DISPLAY_CTRL,
           FIELD_VALUE(PEEK32(CRT_DISPLAY_CTRL), CRT_DISPLAY_CTRL, BLANK, crtdb));
  }
  return 0;
}
...
int hw_sm750_setBLANK(struct lynxfb_output *output, int blank)
{
  ...
  if (output->paths & sm750_panel)
    POKE32(PANEL_DISPLAY_CTRL,
           FIELD_VALUE(PEEK32(PANEL_DISPLAY_CTRL), PANEL_DISPLAY_CTRL, DATA,
                       pps));
  return 0;
}
