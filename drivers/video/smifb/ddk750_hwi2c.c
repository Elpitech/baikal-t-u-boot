...
int sm750_hw_i2c_init(unsigned char bus_speed_mode)
{
	...
	value = PEEK8(I2C_CTRL) & ~(I2C_CTRL_MODE | I2C_CTRL_EN);
	if (bus_speed_mode)
		value |= I2C_CTRL_MODE;
	value |= I2C_CTRL_EN;
...
void sm750_hw_i2c_close(void)
{
	...
	value = PEEK8(I2C_CTRL) & ~I2C_CTRL_EN;
	POKE8(I2C_CTRL, value);
...
static long hw_i2c_wait_tx_done(void)
{
	...
	while (!(PEEK8(I2C_STATUS) & I2C_STATUS_TX) && (timeout != 0))
		timeout--;
 ...
 */
static unsigned int hw_i2c_write_data(unsigned char addr, unsigned int length,
				      unsigned char *buf)
{
	...
	POKE8(I2C_SLAVE_ADDRESS, addr & ~0x01);
	...
	do {
		...
		for (i = 0; i <= count; i++)
			POKE8(I2C_DATA0 + i, *buf++);

		/* Start the I2C */
		POKE8(I2C_CTRL, PEEK8(I2C_CTRL) | I2C_CTRL_CTRL);
 ...
 */
static unsigned int hw_i2c_read_data(unsigned char addr, unsigned int length,
				     unsigned char *buf)
{
	unsigned char count, i;
	unsigned int total_bytes = 0;
