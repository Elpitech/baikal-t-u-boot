...
int xhci_alloc_virt_device(struct usb_device *udev)
{
	...
	byte_64 = __pa(virt_dev->out_ctx->bytes);
					...
					struct xhci_hcor *hcor)
{
	...
	trb_64 = __pa(ctrl->cmd_ring->first_seg->trbs);
	...
	deq = __pa(ctrl->event_ring->dequeue);
	...
	val_64 |= (__pa(ctrl->erst.entries) & ~ERST_PTR_MASK);
...
void xhci_setup_addressable_virt_dev(struct usb_device *udev)
{
	...
	trb_64 = __pa(virt_dev->eps[0].ring->first_seg->trbs);
