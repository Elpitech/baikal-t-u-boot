/*
 * File:        cmd_cpu.с
 * Description: Implementation of cpu tune command
 * Created:     28 Apr 2015
...
static int do_cpu_freq(int argc, char * const argv[])
{
	...
	if (freq > 1200) {
		puts("The CPU frequency higher than 1200MHz is not supported\n");
		return CMD_RET_FAILURE;
	}
	/* Check low freq limit */
	if (freq < 100) {
		puts("The CPU frequency less than 100MHz is not supported\n");
		return CMD_RET_FAILURE;
	}
