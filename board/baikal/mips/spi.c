...
int llenv32_spi_transfer (int port, int line, void *tx_, int tx_len, void *rx_, int rx_len)
{
    ...
    switch(SPI_REG_CTRLR0(port).tmod){
        ...
        case SPI_TMOD_TO:
            ...
            /* wait... */
            mdelay(SPI_BAUDR_SCKDV);                        /* replace with operation status check? */
...
int llenv32_spi_id (int port, int line, void *data, uint32_t size)
{
    return llenv32_spi_exec (port, line, SPI_FLASH_RDID, 0, data, size);
}

int llenv32_spi_bootid (uint8_t *data, uint32_t size)
{
    int port = 0, line = 0, err = 0;

    llenv32_boot_ctrl(1);
    mdelay(100);

    void *cfg = llenv32_spi_store_cfg(port);
    if (!cfg)
        goto exit;
    llenv32_spi_init(port);

    size = size > JEDEC_DATA? JEDEC_DATA : size;
    err |= llenv32_spi_id (port, line, data, size);
    err |= data[0] == 0x00;
    err |= data[0] == 0xFF;

exit:
    err |= llenv32_spi_restore_cfg (port, cfg);
    llenv32_boot_ctrl(0);
    return err;
}
